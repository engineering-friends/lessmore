    ...
    for field, value in _pb_fields_and_values(pb, including_default_value_fields):
        if field.message_type and field.message_type.has_options and field.message_type.GetOptions().map_entry:
            ...
            for k, v in value.items():
                result_dict[field.name][k] = adapter(v)
            continue
        adapter = _field_value_adapter(
            pb=pb,
            field=field,
        ...
        if field.label == FieldDescriptor.LABEL_REPEATED:
            adapter = repeated(adapter)

        if field.is_extension:
            extensions[str(field.number)] = adapter(value)
            continue

        result_dict[field.name] = adapter(value)
...
def _dict_to_protobuf(pb, value, type_callable_map, strict, ignore_none, use_date_parser_for_fields):
    ...
    for field, input_value, pb_value in fields:
        ...
        try:
            if getattr(pb, field.name) == input_value:
                continue
