...
class GRPCAsyncioClient:
    ...
    async def make_grpc_request(
        ...
        return await getattr(stub_class(self._get_channel()), method)(request=request, timeout=timeout, **kwargs)
...
async def test():
    ...
    print(
        (
            await grpc_client.make_grpc_request(
