...
def build_main_menu(deps: Deps):
    async def start_onboarding(response: Response):
        ...
        await response.ask(
            "5. Теперь твоя задача - убедиться, чтобы он все заполнил! Как сделает, Матвею придет уведомление, после чего он напишет пост о новом участнике, а также поможет ему сделать его первый запрос. На этом онбординг будет завершен, мерси боку! ",
